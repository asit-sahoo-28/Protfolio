...
const Technologies = () => {
  ...
  return (
    <div className="px-6 md:px-16 lg:px-32 py-16 bg-[#0e0e11] transition-colors duration-300 min-h-screen">
      <SectionTitle title="Technologies" />

      <div className="flex flex-col md:flex-row gap-10 mt-12">
        {/* Sidebar */}
        <div className="w-full md:w-1/4 flex md:flex-col overflow-x-auto md:overflow-y-auto gap-4 p-2 bg-[#17171c] rounded-2xl shadow-lg">
          {technologies.map((tech, index) => (
            <button
              key={tech._id || index}
              onClick={() => setSelectItem(index)}
              className={`flex-shrink-0 md:w-full text-left p-3 rounded-lg transition-all duration-300 ${
                selectItem === index
                  ...
                  : "text-gray-400 hover:bg-[#23232a] hover:text-white"
              }`}
            >
              <h1 className="text-base md:text-lg font-semibold">
                {tech.title}
        ...
        {/* Card */}
        <div className="flex-1 bg-[#17171c] rounded-2xl shadow-xl p-8 transition-all duration-500">
          <div className="flex flex-col lg:flex-row items-center lg:items-start gap-8">
            {/* Image */}
            <div className="w-full lg:w-1/3 flex justify-center items-center">
              <div className="w-48 h-48 md:w-64 md:h-64 rounded-xl overflow-hidden bg-[#23232a] flex justify-center items-center shadow-md hover:scale-105 transition-transform duration-500">
                <img
                  src={technologies[selectItem].image}
                  alt={technologies[selectItem].title}
                  className="w-36 h-36 object-contain"
            ...
            <div className="flex-1 text-center lg:text-left">
              ...
              <p className="mb-6 text-base md:text-lg text-gray-400 leading-relaxed">
                {technologies[selectItem].description}
              </p>
              ...
              {technologies[selectItem].link && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-block bg-[#7c3aed] hover:bg-[#8b5cf6] text-white px-5 py-2 rounded-full transition-all duration-300"
                >
                  View More
                </a>
              )}
